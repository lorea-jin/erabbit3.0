<template>
  <HomePanel title="最新专题">
    ...
    <div class="special-list" ref="SpecialRef">
      <div class="special-item" v-for="item in list" :key="item.id">
        <RouterLink to="/">
          <img v-lazyload="item.cover" alt />
          <div class="meta">
            <div class="title ellipsis">{{item.title}}
              <div class="subtitle">{{item.summary}}</div>
            </div>
...
import { findSpecial } from '../api/category.js'
...
export default {
  ...
  setup () {
    ...
    list = useLazyData(SpecialRef, findSpecial)

    return { SpecialRef, list }
  }
}
</script>
